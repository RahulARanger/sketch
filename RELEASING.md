...
4. Protect `main`, require the Quality workflow, and merge changes from `dev` through a pull request.
5. Bump the version in `package.json`, `src-tauri/Cargo.toml`, and `src-tauri/tauri.conf.json` before each merge to `main`. The release tag is `v<version>`, so a version cannot be released twice.
